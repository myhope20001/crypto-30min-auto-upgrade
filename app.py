...
# 상폐/유의/상장 7일 제외
# -----------------------------
def filter_safe_coins(coins):
    safe_coins = []
    for c in coins:
        try:
            info = pyupbit.get_tickers("KRW")
            # 상장일 확인
            df = pyupbit.get_ohlcv(c, "day", count=7)
...
def top100():
    coins = filter_safe_coins(tradable())
    data = []
    for c in coins:
        try:
...
def trade(model):
    ...
    for coin in coins:
        ...
        cur.execute("INSERT INTO trades VALUES(NULL,?,?,?,?,?,?,?,?,?)",
                    (datetime.now(), coin, price, qty, "BUY", invest, 0, 0))
        conn.commit()
    ...
    for coin, pos in positions.items():
        price = pyupbit.get_current_price(coin)
        profit = price*pos["qty"] - pos["buy_price"]*pos["qty"]
        ...
        if prob < 0.45 or profit > 0.08*pos["buy_price"]*pos["qty"] or profit < -0.03*pos["buy_price"]*pos["qty"]:
            ...
            cur.execute("INSERT INTO trades VALUES(NULL,?,?,?,?,?,?,?,?,?)",
                        (datetime.now(), coin, price, qty, "SELL", qty*price, profit, profit_percent))
            conn.commit()
...
# Streamlit UI
# -----------------------------
st.title("AI Crypto Trader (5분마다 백그라운드 학습/매매)")
...
for coin, pos in positions.items():
    ...
    rows.append({
        ...
        "price": price,
        "trade_value": pos["buy_price"]*pos["qty"],
        "profit": profit,
...
st.subheader("총합")
st.write(f"총 매수 금액: {total_buy:,.0f} 원")
st.write(f"총 매도 금액: {total_sell:,.0f} 원")
st.write(f"총 이익 금액: {total_profit:,.0f} 원")
st.write(f"총 이익률: {total_profit_percent:.2f} %")
